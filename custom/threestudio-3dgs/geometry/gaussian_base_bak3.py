...
class GaussianBaseModel(BaseGeometry, GaussianIO):
    ...
    class Config(BaseGeometry.Config):
        ...
        point_tag_path: str = ""   # .npy or .npz (key="tags"), shape [N]; 1=car, 2/3=background
    ...
    def configure(self) -> None:
        ...
        self.register_buffer("_point_tag", torch.empty(0, dtype=torch.long), persistent=False)
        ...
        # Support Initialization from OpenLRM, Please see https://github.com/Adamdad/threestudio-lrm
            ...
            self.create_from_pcd(pcd, 10)
            self.training_setup()
    def _ensure_point_tags_loaded(self):
        if not hasattr(self, "_xyz") or self._xyz.numel() == 0:
            return
        n_pts = int(self._xyz.shape[0])
        if self._point_tag.numel() == n_pts:
            return

        tag_path = str(getattr(self.cfg, "point_tag_path", "") or "").strip()
        if tag_path == "":
            raise RuntimeError(
                f"point_tag_path is empty, but point-routed training requires point tags "
                f"(expected {n_pts} tags)."
            )

        loaded = np.load(tag_path, allow_pickle=False)
        if isinstance(loaded, np.lib.npyio.NpzFile):
            if "tags" not in loaded:
                raise RuntimeError(
                    f"point_tag npz file must contain key 'tags': {tag_path}"
                )
            tags = loaded["tags"]
        else:
            tags = loaded
        ...
        if tags.ndim != 1:
            raise RuntimeError(
                f"point_tag file must be 1D, got shape={tags.shape}"
            )
        if tags.shape[0] != n_pts:
            raise RuntimeError(
                f"point_tag size mismatch: file has {tags.shape[0]}, geometry has {n_pts}"
            )

        self._point_tag = torch.from_numpy(tags).long().to(self._xyz.device)

    @property
    def car_point_mask(self) -> torch.Tensor:
        self._ensure_point_tags_loaded()
        if self._point_tag.numel() != self._xyz.shape[0]:
            raise RuntimeError(
                f"_point_tag size mismatch: {self._point_tag.numel()} vs {self._xyz.shape[0]}"
            )
        return self._point_tag == 1

    @property
    def bg_point_mask(self) -> torch.Tensor:
        self._ensure_point_tags_loaded()
        if self._point_tag.numel() != self._xyz.shape[0]:
            raise RuntimeError(
                f"_point_tag size mismatch: {self._point_tag.numel()} vs {self._xyz.shape[0]}"
    ...
    def _expand_point_mask_like(self, point_mask_1d: torch.Tensor, tensor: torch.Tensor) -> torch.Tensor:
        shape = [point_mask_1d.shape[0]] + [1] * (tensor.ndim - 1)
        return point_mask_1d.view(*shape).to(device=tensor.device, dtype=tensor.dtype)

    def mask_param_grad_inplace(self, keep_mask_1d: torch.Tensor, names=None):
        if names is None:
            names = ["_features_dc", "_features_rest"]
        for name in names:
            p = getattr(self, name, None)
            if p is None or p.grad is None:
                continue
            gmask = self._expand_point_mask_like(keep_mask_1d, p.grad)
            p.grad.mul_(gmask)

    def clone_current_grads(self, names=None):
        if names is None:
            names = ["_features_dc", "_features_rest"]
        out = {}
        for name in names:
    ...
    def assign_grads_from_dict(self, grad_dict):
    ...
    def add_grads_from_dict(self, grad_dict):
        for name, g in grad_dict.items():
            p = getattr(self, name, None)
    ...
    def prune_points(self, mask):
        ...
        if self._point_tag.numel() == valid_points_mask.numel():
            self._point_tag = self._point_tag[valid_points_mask]
