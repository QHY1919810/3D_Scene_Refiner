...
class GaussianSplatting(BaseLift3DSystem):
    ...
    #     Returns xs, ys, in_frustum using the matrix orientation that matches visibility best.
    #     """
    ...
    #         frustum = valid & (x_ndc >= -1.0) & (x_ndc <= 1.0) & (y_ndc >= -1.0) & (y_ndc <= 1.0)
    #         xs = ((x_ndc + 1.0) * 0.5 * max(int(cam.image_width) - 1, 1)).round().long()
    #         ys = ((1.0 - (y_ndc + 1.0) * 0.5) * max(int(cam.image_height) - 1, 1)).round().long()
    #         xs = xs.clamp(0, max(int(cam.image_width) - 1, 0))
    #         ys = ys.clamp(0, max(int(cam.image_height) - 1, 0))
    #         score = frustum.sum()
    #         if visibility_hint is not None and visibility_hint.numel() == frustum.numel():
    #             score = (frustum & visibility_hint.bool()).sum()
    #         return xs, ys, frustum, score

    #     xs1, ys1, fr1, sc1 = _run(cam.full_proj_transform)
    #     xs2, ys2, fr2, sc2 = _run(cam.full_proj_transform.T)
    ...
    def _project_points_to_pixels(
        ...
        xyz: torch.Tensor,
        cam: Camera,
        visibility_hint: Optional[torch.Tensor] = None,
    ):
        """
        Project world-space points to pixel coordinates for the current camera.

        Returns:
            xs, ys: rounded integer pixel coords (only meaningful where valid_image=True)
            valid_image: point projects to a valid pixel inside image
            valid_front: point is numerically valid and in front / in NDC depth range
        """
        xyz1 = torch.cat([xyz, torch.ones_like(xyz[:, :1])], dim=-1)
        ...
        def _run(mat: torch.Tensor):
            ...
            # must be inside canonical view volume
            valid_front = (
                valid
                & torch.isfinite(x_ndc)
                & torch.isfinite(y_ndc)
            ...
            valid_image = (
                valid_front
                & (xs >= 0)
                & (xs < int(cam.image_width))
                & (ys >= 0)
                & (ys < int(cam.image_height))
            )
    ...
    #     xs, ys, frustum = self._project_points_to_pixels(xyz, cam, visibility_hint)
    #     visible = frustum
    #     if visibility_hint is not None and visibility_hint.numel() == visible.numel():
    #         visible = visible & visibility_hint

    #     mask2d = mask_in_hw1[..., 0].to(device=xyz.device)
    #     inside = torch.zeros((xyz.shape[0],), dtype=torch.bool, device=xyz.device)
    #     idx = torch.nonzero(visible, as_tuple=False).squeeze(-1)
    #     if idx.numel() > 0:
    #         inside[idx] = mask2d[ys[idx], xs[idx]] > 0.5

    #     point_in = visible & inside
    #     point_out = visible & (~inside)
    ...
    def _compute_dynamic_point_route_masks(self, batch, out, mask_in_hw1: torch.Tensor):
        ...
        # final visible set: only points both projected validly and actually visible in rasterizer
        visible = valid_image
        if visibility_hint is not None and visibility_hint.numel() == visible.numel():
            visible = visible & visibility_hint
    ...
    def training_step(self, batch, batch_idx):
        ...
        points_in = float(point_in_mask.sum().item())
        points_out = float(point_out_mask.sum().item())
        self.log("train/route_points_in", points_in)
        self.log("train/route_points_out", points_out)
        print("########points in, points out ###############\n")
