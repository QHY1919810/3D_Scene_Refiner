...
class GaussianSplatting(BaseLift3DSystem):
    ...
    class Config(BaseLift3DSystem.Config):
        ...
        # Rewrite guidance
        rewrite_guidance_type: str = "qwen-image-rewrite-guidance-low-mem"
        rewrite_guidance: dict = field(default_factory=dict)
    ...
    def configure(self) -> None:
        ...
        # Rewrite guidance / legacy Qwen fallback
        self._qwen_wrapper = None
        self._rewrite_cache: Dict[str, Tuple[int, Any]] = {}
        self.rewrite_guidance = None
        if getattr(self.cfg, "enable_rewrite", False):
            rewrite_guidance_type = str(getattr(self.cfg, "rewrite_guidance_type", "qwen-image-rewrite-guidance-low-mem"))
            rewrite_guidance_cfg = self._build_rewrite_guidance_cfg()
            try:
                importlib.import_module("threestudio.models.guidance.qwen_image_rewrite_guidance_low_mem")
                self.rewrite_guidance = threestudio.find(rewrite_guidance_type)(rewrite_guidance_cfg)
            except Exception as e:
                threestudio.warn(f"Failed to initialize rewrite guidance '{rewrite_guidance_type}', falling back to legacy inline Qwen: {e}")
                self.rewrite_guidance = None
    ...
    def _build_rewrite_guidance_cfg(self) -> dict:
        qcfg = dict(getattr(self.cfg, "qwen", {}) or {})
        user_cfg = dict(getattr(self.cfg, "rewrite_guidance", {}) or {})
        cfg = {
            "diffsynth_repo_root": qcfg.get("diffsynth_repo_root", "/nfs4/qhy/projects/DiffSynth-Studio"),
            "models_cache_root": qcfg.get("models_cache_root", "/nfs4/qhy/projects/DiffSynth-Studio/models/Qwen"),
            ...
            "torch_dtype": str(qcfg.get("torch_dtype", "bfloat16")),
            "update_interval": int(getattr(self.cfg, "rewrite_update_interval", 100)),
            "loss_type": str(getattr(self.cfg, "rewrite_loss_type", "l2")),
            "masked_loss": bool(getattr(self.cfg, "rewrite_masked", True)),
    ...
    def _maybe_dump_render_only(self, render_hwc: torch.Tensor, batch: Dict[str, Any]) -> None:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Optional debug dump of render even when rewrite is disabled (e.g. SDS-only runs).
        if not bool(getattr(self.cfg, "enable_rewrite", False)) and "comp_rgb" in out and out["comp_rgb"].shape[0] > 0:
            self._maybe_dump_render_only(out["comp_rgb"][0], batch)
        ...
        if getattr(self.cfg, "enable_sup", False):
            ...
            self._record_loss_value("loss_rgb", sup["loss_rgb"])


            # HF loss (masked, high-frequency) inside GT mask
            lam_hf = float(self.cfg.loss.get("lambda_hf", 0.0))
            if lam_hf > 0.0:
                if "mask" not in batch or batch["mask"] is None:
                    # Missing mask -> skip HF loss for this batch (do NOT crash training).
                    loss_hf = out["comp_rgb"].sum() * 0.0
                    self.log("train/loss_hf", loss_hf)
                else:
                    loss_hf = masked_high_frequency_loss(
                        pred_rgb_bhwc=out["comp_rgb"].to(dtype=torch.float32),
                ...
                self.log("train/loss_hf", loss_hf)
                loss_total = loss_total + lam_hf * loss_hf
                self._record_loss_value("loss_hf", loss_hf)
        ...
        if getattr(self.cfg, "enable_rewrite", False):
            ...
            if self.rewrite_guidance is not None:
                rewrite_kwargs = dict(batch)
                rewrite_kwargs["view_id"] = view_id
                rewrite_kwargs["edit_image"] = None
                rewrite_kwargs["prompt"] = str(dict(getattr(self.cfg, "qwen", {}) or {}).get("prompt", ""))
                rewrite_out = self.rewrite_guidance(
                    rgb_pred,
                    self.prompt_utils,
                    elevation=batch.get("elevation", torch.zeros((1,), device=self.device)),
                    azimuth=batch.get("azimuth", torch.zeros((1,), device=self.device)),
                    camera_distances=batch.get("camera_distances", torch.ones((1,), device=self.device)),
                    **rewrite_kwargs,
                )
                loss_rw = rewrite_out["loss_rewrite"]
                tgt_rgb = rewrite_out.get("tgt_rgb", None)
                if tgt_rgb is not None:
                    tgt_hwc = tgt_rgb[0].permute(1, 2, 0).to(device=self.device, dtype=torch.float32)
                    self._maybe_dump_render_and_qwen(rgb_pred[0], tgt_hwc, batch)
            else:
                ...
                if getattr(self.cfg, "rewrite_masked", True) and ("mask" in batch):
                    ...
                    if mask.ndim == 2:
                        mask = mask[..., None]
                    mask = mask[..., :1]
                    H, W = int(rgb_pred.shape[1]), int(rgb_pred.shape[2])
                    mask = _resize_mask_to_hw(mask, H, W)
            ...
            self._record_loss_value("loss_rewrite", loss_rw)
