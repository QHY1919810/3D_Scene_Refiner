...
class GaussianSplatting(BaseLift3DSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Skip views without mask instead of crashing.
        if "mask" not in batch or batch["mask"] is None:
            print("[SKIP NO MASK] view_id =", batch.get("view_id", None))
            self.log("train/skip_no_mask", 1.0, on_step=True, prog_bar=True, logger=True)
            opt = self.optimizers()
