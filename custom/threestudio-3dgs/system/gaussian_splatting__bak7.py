...
def _dbg_save_gray(mask_hw1: torch.Tensor, save_path: str):
    if mask_hw1.ndim == 3:
        mask_hw1 = mask_hw1[..., 0]
    x = mask_hw1.detach().float().cpu().numpy()
    x = np.clip(x, 0.0, 1.0)
    x = (x * 255.0).astype(np.uint8)
    Image.fromarray(x).save(save_path)


def _dbg_mask_stats(name: str, x: torch.Tensor):
    if x.ndim == 3:
        x = x[..., 0]
    x = x.detach().float()
    uniq = torch.unique(x)
    uniq_show = uniq[:10].cpu().tolist()
    print(f"### {name} shape ###", tuple(x.shape))
    print(f"### {name} min/max/mean ###", float(x.min().item()), float(x.max().item()), float(x.mean().item()))
    print(f"### {name} unique[:10] ###", uniq_show, "count =", int(uniq.numel()))


def _dbg_save_projected_points(mask_hw1, xs, ys, visible, save_path):
    if mask_hw1.ndim == 3:
        mask_hw1 = mask_hw1[..., 0]
    base = (mask_hw1.detach().float().cpu().numpy() > 0.5).astype(np.uint8) * 255
    vis = np.stack([base, base, base], axis=-1)

    xs_cpu = xs[visible].detach().cpu().numpy()
    ys_cpu = ys[visible].detach().cpu().numpy()

    if len(xs_cpu) > 0:
        stride = max(len(xs_cpu) // 5000, 1)
        for x, y in zip(xs_cpu[::stride], ys_cpu[::stride]):
            vis[y, x, 0] = 255
            vis[y, x, 1] = 0
            vis[y, x, 2] = 0
...
class GaussianSplatting(BaseLift3DSystem):
    ...
    def _project_points_to_pixels(self, xyz: torch.Tensor, cam: Camera, visibility_hint: Optional[torch.Tensor] = None):
        """Project world-space points to pixel coordinates for the current camera.
        Returns xs, ys, valid_image using the matrix orientation that matches visibility best.
        IMPORTANT: do not clamp before validity checks, or out-of-image points will be folded back into the mask.
        """
        xyz1 = torch.cat([xyz, torch.ones_like(xyz[:, :1])], dim=-1)

        def _run(mat: torch.Tensor):
            clip = xyz1 @ mat
            w = clip[:, 3]
            valid = torch.isfinite(w) & (w.abs() > 1e-8)
            ndc = torch.zeros_like(clip[:, :3])
            ndc[valid] = clip[valid, :3] / w[valid, None]
            x_ndc, y_ndc, z_ndc = ndc[:, 0], ndc[:, 1], ndc[:, 2]

            valid_front = (
                valid
                & torch.isfinite(x_ndc) & torch.isfinite(y_ndc) & torch.isfinite(z_ndc)
                & (x_ndc >= -1.0) & (x_ndc <= 1.0)
                & (y_ndc >= -1.0) & (y_ndc <= 1.0)
                & (z_ndc >= -1.0) & (z_ndc <= 1.0)
            )

            xs_f = (x_ndc + 1.0) * 0.5 * float(max(int(cam.image_width) - 1, 1))
            ys_f = (1.0 - (y_ndc + 1.0) * 0.5) * float(max(int(cam.image_height) - 1, 1))
            xs = xs_f.round().long()
            ys = ys_f.round().long()

            valid_image = (
                valid_front
                & (xs >= 0) & (xs < int(cam.image_width))
                & (ys >= 0) & (ys < int(cam.image_height))
            )

            score = valid_image.sum()
            if visibility_hint is not None and visibility_hint.numel() == valid_image.numel():
                score = (valid_image & visibility_hint.bool()).sum()
            return xs, ys, valid_image, score

        xs1, ys1, im1, sc1 = _run(cam.full_proj_transform)
        xs2, ys2, im2, sc2 = _run(cam.full_proj_transform.T)
        if int(sc2.item()) > int(sc1.item()):
            return xs2, ys2, im2
        return xs1, ys1, im1

    @torch.no_grad()
    def _compute_dynamic_point_route_masks(self, batch, out, mask_in_hw1: torch.Tensor):
        """Patch-vote routing.
        For each currently visible point, vote on a local mask patch around the projected center.
        - inside ratio >= in_thresh  -> point_in
        - inside ratio <= out_thresh -> point_out
        - otherwise ignored (boundary/ambiguous)
        """
        cams = _as_list(batch.get("camera", None))
        if len(cams) != 1:
            raise ValueError("dynamic point routing currently expects exactly one camera in the batch.")
        cam = _camera_to_device(cams[0], self.device)

        xyz = self.geometry.get_xyz.detach()
        visibility_hint = None
        if "visibility_filter" in out and len(out["visibility_filter"]) > 0:
            visibility_hint = out["visibility_filter"][0].to(device=xyz.device).bool()

        xs, ys, valid_image = self._project_points_to_pixels(xyz, cam, visibility_hint)
        visible = valid_image

        if torch.any(visible):
            sampled = mask_in_hw1[..., 0][ys[visible], xs[visible]].detach().float()

            print("######## ROUTE SAMPLE DEBUG ########")
            print("### visible count ###", int(visible.sum().item()))
            print("### sampled min/max/mean ###", float(sampled.min().item()), float(sampled.max().item()), float(sampled.mean().item()))
            print("### sampled unique ###", torch.unique(sampled).cpu().tolist())
            print("### sampled > 0.5 ratio ###", float((sampled > 0.5).float().mean().item()))

            print("### xs min/max ###", int(xs[visible].min().item()), int(xs[visible].max().item()))
            print("### ys min/max ###", int(ys[visible].min().item()), int(ys[visible].max().item()))

            _dbg_save_projected_points(
                mask_in_hw1,
                xs,
                ys,
                visible,
                os.path.join("/nfs4/qhy/projects/threestudio/test_datas", f"projected_points_step{int(self.global_step):06d}.png"),
            )
        else:
            print("######## ROUTE SAMPLE DEBUG ########")
            print("### visible count ### 0")


        if visibility_hint is not None and visibility_hint.numel() == visible.numel():
            visible = visible & visibility_hint

        mask2d = mask_in_hw1[..., 0].to(device=xyz.device, dtype=torch.float32)
        H, W = int(mask2d.shape[0]), int(mask2d.shape[1])

        # patch vote hyper-parameters (safe defaults)
        radius = int(getattr(self.cfg, "route_patch_radius", 3))
        in_thresh = float(getattr(self.cfg, "route_in_thresh", 0.7))
        out_thresh = float(getattr(self.cfg, "route_out_thresh", 0.3))

        idx = torch.nonzero(visible, as_tuple=False).squeeze(-1)
        point_in = torch.zeros((xyz.shape[0],), dtype=torch.bool, device=xyz.device)
        point_out = torch.zeros((xyz.shape[0],), dtype=torch.bool, device=xyz.device)

        if idx.numel() == 0:
            return point_in, point_out

        # integer window around each projected center
        x0 = (xs[idx] - radius).clamp(0, W - 1)
        x1 = (xs[idx] + radius).clamp(0, W - 1)
        y0 = (ys[idx] - radius).clamp(0, H - 1)
        y1 = (ys[idx] + radius).clamp(0, H - 1)

        ratios = torch.zeros((idx.numel(),), dtype=torch.float32, device=xyz.device)
        for k in range(idx.numel()):
            patch = mask2d[y0[k]:y1[k] + 1, x0[k]:x1[k] + 1]
            if patch.numel() > 0:
                ratios[k] = patch.mean()

        point_in[idx] = ratios >= in_thresh
        point_out[idx] = ratios <= out_thresh
        return point_in, point_out
    ...
    def training_step(self, batch, batch_idx):
        ...
        if "mask" not in batch or batch["mask"] is None:
            raise RuntimeError("Dynamic point-routed rewrite requires batch['mask'].")
        if not getattr(self.cfg, "enable_rewrite", False):
            raise RuntimeError("Dynamic point-routed rewrite requires enable_rewrite=True.")
        if "rgb" not in batch:
            raise KeyError("Dynamic point-routed rewrite requires batch['rgb'] for HF loss.")

        # This variant intentionally disables shared/global losses. They would update all points
        # and break strict per-point routing.
        forbidden = [
            "lambda_rgb",
            "lambda_silhouette",
            "lambda_position",
        ...
        active_forbidden = [k for k in forbidden if float(self.cfg.loss.get(k, 0.0)) > 0.0]
        if active_forbidden:
            raise RuntimeError(
                "Dynamic point-routed training requires these shared losses to be zero: "
                + ", ".join(active_forbidden)
            )

        rgb_pred = out["comp_rgb"]
        if rgb_pred.shape[0] != 1:
            raise ValueError("Dynamic point-routed rewrite currently expects batch_size=1.")
        ...
        if mask.ndim == 2:
            mask = mask[..., None]
        mask = mask[..., :1]
        if float(mask.max().item()) > 1.5:
            mask = mask / 255.0
        mask_in = _resize_mask_to_hw(mask, H, W)


        print("######## MASK DEBUG ########")
        _dbg_mask_stats("raw_mask", mask)
        _dbg_mask_stats("resized_mask", mask_in)

        debug_dir = os.path.join(self.get_save_dir(), "route_debug")
        os.makedirs(debug_dir, exist_ok=True)

        _dbg_save_gray(mask, os.path.join(debug_dir, f"raw_mask_step{int(self.global_step):06d}.png"))
        _dbg_save_gray(mask_in, os.path.join(debug_dir, f"resized_mask_step{int(self.global_step):06d}.png"))
        mask_in = (mask_in > 0.5).to(dtype=pred.dtype)
        mask_out = 1.0 - mask_in

        

        point_in_mask, point_out_mask = self._compute_dynamic_point_route_masks(batch, out, mask_in)
        route_visible = (point_in_mask | point_out_mask)
        print("### in ###")
        print(float(point_in_mask.sum().item()))
        print("### out ###")
        print(float(point_out_mask.sum().item()))
        self.log("train/route_points_in", float(point_in_mask.sum().item()))
        self.log("train/route_points_out", float(point_out_mask.sum().item()))
        self.log("train/route_points_visible", float(route_visible.sum().item()))
        self.log("train/route_in_ratio", float(point_in_mask.sum().item()) / max(float(route_visible.sum().item()), 1.0))
        ...
        # Branch A: mask-in rewrite + HF -> only points currently projecting into mask
        loss_in_branch = lam_rw * loss_rw_in + lam_hf * loss_hf_in
        if torch.any(point_in_mask):
            self.manual_backward(loss_in_branch, retain_graph=True)
            self.geometry.mask_param_grad_inplace(keep_mask_1d=point_in_mask, names=routed_names)
            in_grads = self.geometry.clone_current_grads(names=routed_names)
        else:
            in_grads = {name: None for name in routed_names}
        opt.zero_grad(set_to_none=True)

        # Branch B: mask-out rewrite -> only points currently projecting outside mask
        loss_out_branch = lam_rw * loss_rw_out
        if torch.any(point_out_mask):
            self.manual_backward(loss_out_branch, retain_graph=True)
            self.geometry.mask_param_grad_inplace(keep_mask_1d=point_out_mask, names=routed_names)
            out_grads = self.geometry.clone_current_grads(names=routed_names)
        else:
            out_grads = {name: None for name in routed_names}
        opt.zero_grad(set_to_none=True)

        # Merge routed grads and step
        self.geometry.assign_grads_from_dict(in_grads)
        self.geometry.add_grads_from_dict(out_grads)
        opt.step()
        opt.zero_grad(set_to_none=True)

        loss_total = loss_in_branch.detach() + loss_out_branch.detach()
