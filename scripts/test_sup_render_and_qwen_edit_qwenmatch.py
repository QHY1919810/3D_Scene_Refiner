...
def main():
    ...
    ap.add_argument("--zero_cond_t", action="store_true", help="Qwen-Image-Edit-2511 recommended flag")
    ap.add_argument("--no_zero_cond_t", action="store_true", help="Disable zero_cond_t")
    ap.add_argument("--edit_image_auto_resize", action="store_true", help="Let pipeline auto resize edit image")
    args = ap.parse_args()

    if args.no_zero_cond_t:
        args.zero_cond_t = False
    elif not args.zero_cond_t:
        # default ON for 2511 unless explicitly disabled
        args.zero_cond_t = True
    ...
    for batch in loader:
        ...
        out = pipe(
            ...
            height=int(qwen_h),
            width=int(qwen_w),
            edit_image_auto_resize=bool(args.edit_image_auto_resize),
            zero_cond_t=True,
        )
